...
import { IDopNFT } from "./IDopNFT.sol";

/// @title DopClaims contract
/// @notice Implements the claiming of Dop Token and NFT
/// @dev The claims contract allows you to claim dop tokens and nfts
contract DopClaims is Ownable, ReentrancyGuard {
    ...
    /// The address of dop token
    IERC20 public immutable DOP;

    /// The address of dop nft contract
    IDopNFT public immutable dopNFT;

    /// The address will distribute dop tokens
    ...
    /// @notice Gives info of user's dop token claim
    mapping(address => bool) public isClaimed;

    /// @notice Gives info about address's permission
    mapping(address => bool) public blacklistAddress;

    /// @dev Emitted when address of dop wallet is updated
    event DopWalletUpdated(address indexed prevAddress, address indexed newAddress);

    /// @dev Emitted when dop token are claimed
    event Claimed(address indexed by, uint256 dopAmount, uint256[] indexed ids, uint256[] indexed quantity);
    ...
    /// @dev Constructor
    /// @param dopToken The address of dop token
    /// @param dopNFTAddress The address of dop nft contract
    /// @param owner The address of the owner wallet
    /// @param dopWalletAddress The address of the wallet that which transfer tokens
    /// @param signerAddress The address of the signer wallet
    /// @param merkleRoot The merkle root of the tree
    constructor(
        IERC20 dopToken,
        IDopNFT dopNFTAddress,
        address owner,
        address dopWalletAddress,
        address signerAddress,
        bytes32 merkleRoot
    ) Ownable(owner) {
        if (
            address(dopToken) == address(0) ||
            address(dopNFTAddress) == address(0) ||
            dopWalletAddress == address(0) ||
        ...
        DOP = dopToken;
        dopNFT = dopNFTAddress;
        dopWallet = dopWalletAddress;
        signerWallet = signerAddress;
        root = merkleRoot;
    }

    /// @notice Claims dop tokens and nfts only when `claimNFT` is true
    /// @param amountToClaim The dop token amount to claim
    ...
    /// @param isKycRequired The user requires KYC verification or not
    /// @param v The `v` KYC signature parameter
    ...
    function claimDop(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, amountToClaim, ids, quantity, isKycRequired));
        ...
        DOP.safeTransferFrom(dopWallet, msg.sender, amountToClaim);
        if (claimNFT && ids.length > 0) {
            dopNFT.mint(msg.sender, ids, quantity);
        }

        emit Claimed({ by: msg.sender, dopAmount: amountToClaim, ids: ids, quantity: quantity });
    }

    /// @notice Changes dop wallet to a new address
    /// @param newDopWallet The address of the new dop wallet
    function changeDopWallet(address newDopWallet) external checkAddressZero(newDopWallet) onlyOwner {
        address oldWallet = dopWallet;
        if (oldWallet == newDopWallet) {
            revert IdenticalValue();
        }

        emit DopWalletUpdated({ prevAddress: oldWallet, newAddress: newDopWallet });
    ...
    function _verifySign(
        ...
        bytes32 encodedMessageHash = keccak256(abi.encodePacked(msg.sender, dopTokenAmount, ids, quantity));
